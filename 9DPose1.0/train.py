...
def train(hyp,                                      
    ...
    if pretrained:
        ...
        if(os.path.basename(weights)=='last.pt'):                      
            if(ckpt['epoch']==-1):
                ckpt['epoch']=epochs-1
    ...
    train_loader, dataset = create_dataloader(train_path, imgsz, batch_size // WORLD_SIZE, gs, single_cls,
                                              ...
                                              prefix=colorstr('train: '), shuffle=True,save_dir=save_dir)
    ...
    nl = de_parallel(model).get_module_byname('Detect').nl                                              
    hyp['box'] *= 3 / nl                   
    hyp['cls'] *= nc / 80 * 3 / nl                               
    ...
    for epoch in range(start_epoch, epochs):                                                                            
        ...
        if opt.image_weights:
            ...
            neg_alpha = hyp.get('neg_alpha', 0.03)          
        ...
        LOGGER.info(('\n' + '%10s' * 11) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'dir', 'abc', 'z', 'pabc', 'labels', 'img_size'))
        if RANK in [-1, 0]:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--weights', type=str, default=ROOT / 'runs/train/exp_kitti48/weights/best.pt', help='initial weights path')
    parser.add_argument('--cfg', type=str, default=ROOT / 'models/yolov5s-kitti.yaml', help='model.yaml path')
...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and not check_wandb_resume(opt) and not opt.evolve:                             
        ...
        if os.path.exists(opt.data):
            ...
            print(f'\033[91mdata_path:{opt.data} not exist.注意路径大小写.\033[0m')
