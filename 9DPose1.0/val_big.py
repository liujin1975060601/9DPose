...
def read_image_label(im_file, pts_file):                                  
    ...
    if nl:
        ...
        boxs = np.clip(l[:, 1:], 0, 1)                                             
        l = np.concatenate((clss, boxs), axis=1)                        
...
def run(data,
    ...
    for image_path, label_path in tqdm(zip(image_files, label_files), desc=s, total=len(image_files)):
        ...
        targets[:, 0] = torch.from_numpy(label[:, 0])           
        targets[:, 1:] = torch.from_numpy(label[:, 1+4:])                
        ...
        targets[:, 1:] *= torch.Tensor([width, height, width, height, width, height, width, height]).to(device)             
        labels = targets                  
        pred = out                               
        nl = len(labels)
        ...
        if len(pred) == 0:
            if nl:
                ...
                continue
        if nl:
            correct = process_batch_poly(pred, labels, iouv)                                                                  
            if plots:
